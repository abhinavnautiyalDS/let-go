...
export default function Scene01() {
  ...
  // ─── COMPOSITOR RESOLUTION (MOBILE OPTIMISED) ─────────────────────
  ...
  const initCompositor = () => {
    ...
    const render = () => {
      ...
      if (timeChanged && !videoEndedRef.current && !compositorEnded.current) {
        ...
        // Vertical diffusion – skipped on mobile
        if (!isMobile) {
          for (let y = 0; y < h; y++) {
            for (let x = 0; x < w; x++) {
              const idx = y * w + x;
          ...
          // On mobile, just copy horizontal result to mask (no vertical blur)
          for (let i = 0; i < SOURCE_W * SOURCE_H; i++) {
            mask[i] = temp[i];
          }
        }
